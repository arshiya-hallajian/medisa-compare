...
export const EachProduct = ({row, updateW, listHandler, multiP, desc}) => {
        ...
        let show_Price, price_label, offer_price, offerVariantBox_price,
            ...
            eachVariantPrice, BoxVariantPrice ,newdesc,medisaName
    if (row.medisa && row.medisa[0]) {

        newdesc = row.desc !== '' ? row.desc : 'No Description';
        medisaName = row.medisa && row.medisa[0].name.split(' ').slice(0, 3).join(' ');
